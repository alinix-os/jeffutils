    ...
    if candidates.is_empty() {
        ...
        loop {
            ...
            for (j, c) in candidates.iter().enumerate() {
                ...
                let sep_w = if j > 0 { 2 } else { 0 };
                if line_w + sep_w + text_w + 6 > cols {
                    let remaining = candidates.len() - j;
                    s.out.write_and_flush(&format!("  (+{})", remaining))?;
                    break;
                }
                if sep_w > 0 {
                    s.out.write_and_flush("  ")?;
                ...
                if j == i {
                    s.out.write_and_flush("\x1b[7m")?;
                    s.out.write_and_flush(text)?;
                    s.out.write_and_flush("\x1b[0m")?;
                } else {
                    s.out.write_and_flush(text)?;
                }
                line_w += sep_w + text_w;
            ...
            s.out.write_and_flush("\r\x1b[A")?;

            cmd = s.next_cmd(input_state, rdr, true, true)?;
            match cmd {
                Cmd::Complete
                | Cmd::LineDownOrNextHistory(_)
                | Cmd::NextHistory => {
