...
fn usage() {
    eprintln!("Usage: blake2 [-l BITS] [-c] [FILE...]");
...
fn version() {
    println!("blake2 0.1.0");
    process::exit(0);
}

fn main() {
if std::env::args().skip(1).any(|a| a == "--version" || a == "-v") { jutils_core::print_version("blake2", env!("CARGO_PKG_VERSION")); std::process::exit(0); }
